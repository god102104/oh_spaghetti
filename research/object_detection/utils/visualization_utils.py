...
def draw_bounding_box_on_image_array(image,
  ...
  """
  image_pil = Image.fromarray(np.uint8(image)).convert('RGB')
  data = draw_bounding_box_on_image(image_pil, ymin, xmin, ymax, xmax, color,
...
def visualize_boxes_and_labels_on_image_array(
  ...
  if scores is None or scores[0] > min_score_thresh:
    ...
    if scores is None:
      ...
      if classes[0] in six.viewkeys(category_index):
        class_name = category_index[classes[0]]['name']
      else:
        class_name = 'N/A'
      display_str = str(class_name)
      score = round(100*scores[0])
      box_to_display_str_map[box].append(display_str)
      box_to_color_map[box] = STANDARD_COLORS[classes[0] % len(STANDARD_COLORS)]
  print('{}: {}%'.format(display_str, round(100*scores[0])))
  data.append(class_name)
  # Draw all boxes onto image.
  for box, color in box_to_color_map.items():
    ymin, xmin, ymax, xmax = box
    data.append(draw_bounding_box_on_image_array(
        image,
        ymin,
        xmin,
        ymax,
